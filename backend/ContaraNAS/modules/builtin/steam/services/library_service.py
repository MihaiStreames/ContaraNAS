...
class SteamLibraryService:
    ...
    def initialize(self) -> bool:
        ...
        self._steam_path = SteamPathService.find_steam_path()
        if not self._steam_path:
            logger.error("Steam installation not found")
            return False

        if not SteamPathService.validate_steam_path(self._steam_path):
            logger.error("Invalid Steam installation")
            return False

        self.parsing_service = SteamParsingService(self._steam_path)

        # Load library paths
        self._library_paths = self.parsing_service.get_library_paths()
        if not self._library_paths:
            logger.error("No Steam libraries found")
            return False

        logger.info(f"Steam library service initialized: {len(self._library_paths)} libraries")
        return True

    def get_library_paths(self) -> list[Path]:
        """Get all Steam library paths"""
        return self._library_paths.copy()
