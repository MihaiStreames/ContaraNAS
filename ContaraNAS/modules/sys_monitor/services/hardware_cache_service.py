...
class HardwareCacheService:
    ...
    def __inif__(self):
        ...
        self._cache_file = self._cache_dir / "hardware_cache.json"
        self._boot_time = psutil.boot_time()

    def needs_refresh(self) -> bool:
        """Check if cache needs refresh (based on system boot time)"""
        if not self._cache_file.exists():
            logger.warning("Hardware cache does not exist")
            return True

        cache_data = load_json(self._cache_file)
        if not cache_data:
            logger.warning("Hardware cache is corrupted or empty")
            return True

        cached_boot_time = cache_data.get("boot_time")
        if cached_boot_time is None:
            logger.warning("No boot time in cache")
            return True
        ...
        logger.debug("Hardware cache is still valid")
        return False
    ...
    def get_or_collect_hardware_info(self, collect_fn: callable) -> dict[str, Any]:
        ...
        if not self.needs_refresh():
            ...
            if cached_data:
                logger.info("Using cached hardware information")
                return cached_data

        logger.info("Collecting fresh hardware information (may require sudo)")
